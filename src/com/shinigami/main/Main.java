...
public class Main extends JavaPlugin {
	...
	public void onEnable() {
		...
		shinigamiLife.load();
		ShinigamiLife.log("--------------------");
		ShinigamiLife.log("MINECRAFTLIFE loaded!");
		ShinigamiLife.log("--------------------");
	}
	...
	public void onDisable() {
		...
		ShinigamiLife.log("--------------------");
		ShinigamiLife.log("MINECRAFTLIFE unloaded!");
		ShinigamiLife.log("--------------------");
	}
	...
	public boolean onCommand(CommandSender sender, Command command,
		...
		if(sender instanceof Player){
			...
			if(command.getName().equals("mclife")){
					...
					if(p.getGameMode() == GameMode.CREATIVE){
						if(args[1].equals("give")){
							if(args[2] != null && args[3] != null){
								...
								sessionT.setMoneyPocket(sessionT
										.getMoneyPocket() + money);
			...
			}else if(command.getName().equals("unload")){
				if(p.getGameMode() == GameMode.CREATIVE){
					shinigamiLife.unload();
					p.sendMessage(Colors.RED + "Unloaded!");
					return true;
				}else{
					p.sendMessage(Colors.RED + "Du hast keine Berechtigung dazu!");
					return false;
				...
				if(p.getGameMode() == GameMode.CREATIVE){
					shinigamiLife.load();
					p.sendMessage(Colors.GREEN  + "Loaded!");
					return true;
				}else{
					p.sendMessage(Colors.RED + "Du hast keine Berechtigung dazu!");
					return false;
